...
describe("AuthProvider", () => {
  ...
  it("uses Basic auth with base64 credentials", async () => {
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      text: () =>
        Promise.resolve(JSON.stringify({ access_token: "t", expires_in: "3599" })),
    } as Response);
  ...
  it("refreshes token after clearCache", async () => {
    ...
    fetchMock
      .mockResolvedValueOnce({
        ok: true,
        text: () =>
          Promise.resolve(JSON.stringify({ access_token: "token1", expires_in: "3599" })),
      } as Response)
      .mockResolvedValueOnce({
        ok: true,
        text: () =>
          Promise.resolve(JSON.stringify({ access_token: "token2", expires_in: "3599" })),
      } as Response);
