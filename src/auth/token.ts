...
import { MpesaAuthError } from "../errors/index.js";
...
export async function fetchToken(
  ...
  if (!response.ok) {
    ...
    throw new MpesaAuthError(message, { responseBody: data, cause: new Error(String(data)) });
  }

  const result = data as TokenResult;
  if (!result?.access_token) {
    throw new MpesaAuthError("Invalid token response: missing access_token", { responseBody: data });
  }
